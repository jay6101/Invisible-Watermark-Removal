...
def main():
    ...
    # Generate random binary list of length 100

    if args.save_dir is not None:
        if not os.path.exists(args.save_dir):
            os.makedirs(args.save_dir)
        size = (width, height)
        for j,filename in enumerate(tqdm(files_list)):
            secret = np.random.randint(0, 2, 100).tolist()
            secret_inv = [int(1-x) for x in secret]
            image = Image.open(filename).convert("RGB")
            image = np.array(ImageOps.fit(image,size),dtype=np.float32)
            image /= 255.
            os.makedirs(args.save_dir + '/' + filename.split('/')[-1].split('.')[0].split('_')[0], exist_ok=True)
            for i in range(2):
                if i == 0:
                    feed_dict = {input_secret:[secret],
                                input_image:[image]}
                else:
                    feed_dict = {input_secret:[secret_inv],
                                input_image:[image]}
                ...
                if i==0:
                    ...
                    im.save(args.save_dir + '/'+filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name+'_hidden.png')
                else:
                    im = Image.fromarray(np.array(rescaled))
                    im.save(args.save_dir + '/'+filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name+'_hidden_inv.png')

                if i==0:
                    im = Image.fromarray(np.squeeze(np.array(residual)))
                    im.save(args.save_dir + '/'+filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name+'_residual.png')
                else:
                    im = Image.fromarray(np.squeeze(np.array(residual)))
                    im.save(args.save_dir + '/'+filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name+'_residual_inv.png')
                
                # Save the secret to a text file
                if i==0:
                    with open(args.save_dir + '/' + filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name + '_secret.txt', 'w') as f:
                        f.write(''.join(map(str, secret)))
                else:
                    with open(args.save_dir + '/' + filename.split('/')[-1].split('.')[0].split('_')[0]+'/'+save_name + '_secret_inv.txt', 'w') as f:
                        f.write(''.join(map(str, secret_inv)))
