...
def main(args):
    ...
    # dataset of prompts / images
    dataset, prompt_key = get_dataset(args)

    # at detection time, assume original prompt is unknown
    tester_prompt = ""
    text_embeddings = pipe.get_text_embedding(tester_prompt)

    # ground truth watermark patch (key)
    ...
    for i in tqdm(range(args.start, args.end)):
        ...
        # get watermark mask in latent space
        ...
        # -----------------------------------
        # 5. DDIM INVERSION (WITH WATERMARK)
        # -----------------------------------
        img_w = (
        ...
        # -----------------------------------
        # 6. EVALUATION (WATERMARK METRICS)
        # -----------------------------------
        ...
        # note the minus sign: repo pushes "higher = more likely watermarked"
        no_w_metrics.append(-no_w_metric)
        w_metrics.append(-w_metric)

        if args.with_tracking:
            if (args.reference_model is not None) and (
                i < args.max_num_log_image
            ):
                table.add_data(
    ...
    print(f"clip_score_mean: {mean(clip_scores) if clip_scores else 0.0}")
    print(f"w_clip_score_mean: {mean(clip_scores_w) if clip_scores_w else 0.0}")
